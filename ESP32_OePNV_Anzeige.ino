...
/* =====================================================
   ...
   ===================================================== */
const char* bvgUrl =
  "https://v6.bvg.transport.rest/stops/900180521/departures?results=4&duration=830&lines=true";
...
void fetchDepartures() {
  ...
  //    departures[i].destination = decodeUtf8(arr[i]["direction"].as<String>());
...
//      Serial.printf("[DEBUG] %d: Linie %s -> %s (%s) in %d min\n",
//        i+1,
//        departures[i].line.c_str(),
//        departures[i].destination.c_str(),
//        departures[i].product.c_str(),
...
void loop() {
  ...
  if(millis() - lastUpdate > 30000){ fetchDepartures(); drawScreen(); lastUpdate = millis(); }
  delay(50);
}
